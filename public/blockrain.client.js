 ...
 */
 ...
 ((function ( $ ) {
   ...
   $.widget('aerolab.blockrain_client', {
     ...
     options: {
       ...
       // Copy
       playText: 'Espera ai que já vão da playy',
       playButtonText: 'Play',
       gameOverText: 'Você Perdeu',
     ...
     _doStart: function() {
       ...
       this._board.se_movendo = false;
       this._board.time_movendo;
     },
     ...
     addline : function(){
       ...
       for (i=0, ilen=game._BLOCK_WIDTH; i<ilen; i++) {
           ...
           //game._filled.add(i, game._BLOCK_HEIGHT - 1, blockTypes[0], 1, null, 1);
     ...
     addSpeed : function(){
       ...
       var game = this;
       game.options.speed = game.options.speed+10;
 
     },

     clientData : function(data){
        var game = this;
        var {x, y, blockType, blockVariation, blockIndex, blockRotation, falling} = data;
        game._filled.add(x, y, blockType, blockVariation, blockIndex, blockRotation, falling);
       ...
       function Shape(game, orientations, symmetrical, blockType) {
         ...
         $.extend(this, {
           ...
           rotate: function(direction) {
             game._board.se_movendo = true;
             clearInterval(game._board.time_movendo)
             game._board.time_movendo = setTimeout(()=>{
               game._board.se_movendo = false;
               game._board.animate();
               game._board.renderChanged = true;
               console.log('saiu');
             }, 500)
     ...
     _SetupFilled: function() {
       ...
       this._filled = {
         ...
         upLine : function(){
           let total = game._BLOCK_WIDTH*(game._BLOCK_HEIGHT);
           let random = Math.floor((Math.random() * game._BLOCK_WIDTH) + total - game._BLOCK_WIDTH);
 
           for (var i=0; i < total; i++) {
           
             this.data[i-game._BLOCK_WIDTH] = this.data[i];
 
             if(i < total && i > total - game._BLOCK_WIDTH-1){
         ...
         _popRow: function(row_to_pop) {
           console.log('Pop', row_to_pop)
           for (var i=game._BLOCK_WIDTH*(row_to_pop+1) - 1; i>=0; i--) {
             this.data[i] = (i >= game._BLOCK_WIDTH ? this.data[i-game._BLOCK_WIDTH] : undefined);
           }
         },
 
         _addRow: function(len) {
           console.log('ADD', game._BLOCK_HEIGHT);
           var total = game._BLOCK_WIDTH*(game._BLOCK_HEIGHT)
           for (var i=total - 1; i>= 0; i--) {
             
             //this.data[i + game._BLOCK_HEIGHT] = this.data[i];
         ...
         draw: function() {
           ...
           game.options.onRender.call(game.element, this.data);
     ...
     _SetupBoard: function() {
       ...
       this._board = {
         ...
         drawBlock: function(x, y, blockType, blockVariation, blockIndex, blockRotation, falling) {
           ...
           console.log('testeeee', { x, y, blockType, blockVariation, blockIndex, blockRotation, falling })
